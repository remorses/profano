...
// by self-time in the terminal. Designed for AI agents and humans who want
// quick profiling insights without opening a browser.
...
cli
  .command('<files...>', 'Analyze .cpuprofile files and print top functions by self-time')
  .option(
    '-n, --limit [limit]',
    z.number().default(30).describe('Number of top functions to show'),
  )
  .example('# Analyze a single profile')
  ...
  .action((files, options) => {
    ...
    for (const filePath of resolved) {
      ...
      console.log(formatTable({ ...result, limit: options.limit }))
    }
  })
